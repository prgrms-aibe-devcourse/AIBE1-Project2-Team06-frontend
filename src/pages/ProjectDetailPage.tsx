...
const cultureFitTypeDetailDescription: { [key: string]: string } = {
  ...
  PRACTICAL:
    "효율성과 실용성을 중시하며, 명확한 역할 분담과 능력에 따른 효율적인 업무 배분을 선호합니다. 문제가 생겼을 때는 스스로 고민한 뒤 해결책을 공유하는 방식으로 일하며, 팀원들과의 소통도 중요시합니다.",
  HARMONY:
    "조화와 균형을 중시하며, 팀원들 간의 조화로운 관계를 유지하기 위해 노력합니다. 독립적인 작업 스타일과 팀워크 사이에서 균형을 찾아가며, 갈등을 최소화하고 팀의 전체적인 목표 달성을 위해 협력하는 성향을 가지고 있습니다.",
  DIRECTIVE:
    "명확한 지시와 체계적인 업무 분배를 통해 프로젝트를 이끌어가는 것을 선호합니다. 목표 지향적이며 효율적인 업무 진행을 위해 구체적인 방향을 제시하고 팀원들의 역할을 명확히 구분합니다.",
};
